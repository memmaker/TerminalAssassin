...
func (m *GridMap[ActorType, ItemType, ObjectType]) SetDownedActorToActive(a ActorType) {
	pos := a.Pos()
	m.Cells[pos.Y*m.MapWidth+pos.X].DownedActor = nil
	m.RemoveDownedActor(a)
	m.AllActors = append(m.AllActors, a)
	if m.IsActorAt(pos) {
		m.displaceActor(a)
		return
	}
	m.Cells[pos.Y*m.MapWidth+pos.X] = m.Cells[pos.Y*m.MapWidth+pos.X].WithActor(a)
}
