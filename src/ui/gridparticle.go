...
func (s *CursorParticle) Draw(grid console.CellInterface) {
    ...
    // Erase the previous cell when the cursor has moved backwards, but not when it has moved forwards, to create a trailing effect
    if s.OldPos.X != s.Pos.X {
        if s.OldPos.X > s.Pos.X {
            grid.SetHalfWidth(s.OldPos, common.Cell{Rune: ' ', Style: common.DefaultStyle})
        }
        s.OldPos = s.Pos
    }
    if s.Color.V <= 0.0 {
