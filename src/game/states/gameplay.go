...
type GameStateGameplay struct {
	...
	lastHolsteredItem *core.Item // player-only holster slot
...
func (g *GameStateGameplay) holsterItem(player *core.Actor) {
	if player.EquippedItem == nil {
		if g.lastHolsteredItem != nil && player.Inventory.Contains(g.lastHolsteredItem) {
			player.EquippedItem = g.lastHolsteredItem
			g.lastHolsteredItem = nil
		}
		return
	}
	if player.EquippedItem.IsBig {
		return
	}
	g.lastHolsteredItem = player.EquippedItem
	player.EquippedItem = nil
}
