...
const maxBowLosLength = 5 // tiles shown in LoS indicator at full stick deflection

// aimBowFromPad handles bow-specific gamepad aiming.
// The LoS indicator points in the direction the stick is pulled.
...
func (g *GameStateGameplay) aimBowFromPad(player *core.Actor, xAxis, yAxis float64) {
	...
	aimTarget := geometry.VectorInDirectionWithLength(player.FoVSource(), xAxis, yAxis, losLength)

	// Store pull vector and grid-snapped endpoint for use at release.
...
func (g *GameStateGameplay) AdjustPlayerAim(aimPosInWorld geometry.Point) {
	...
	a.LookDirection = geometry.DirectionVectorToAngleInDegrees(aimPosInWorld.Sub(a.Pos()))
	aimDistance := a.AimDistance()
	aimDistanceSquared := aimDistance * aimDistance

	g.TargetLoS = geometry.LineOfSight(a.Pos(), aimPosInWorld, func(p geometry.Point) bool {
		return geometry.DistanceSquared(p, a.Pos()) < aimDistanceSquared
	})
	return
}
