...
func (f *FrenzyMovement) NextAction() core.AIUpdate {
	...
	if f.elapsedSeconds >= frenzyDurationSeconds {
		f.exitFrenzy(person)
...
func (f *FrenzyMovement) handleMelee(person *core.Actor) core.AIUpdate {
	...
	if person.EquippedItem != nil && person.EquippedItem.Type.HasMeleeAction() {
		...
		game.ApplyStimulusToActor(f.target, core.NewEffectSourceFromActor(person),
			stimuli.Stim{StimType: stimuli.StimulusBluntDamage, StimForce: 40})
	}
