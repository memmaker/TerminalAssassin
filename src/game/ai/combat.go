...
func (t *CombatMovement) exitCombat(person *core.Actor) {
	...
	if person.AI.HasSchedule() {
		...
		ai.SetState(&ScheduledMovement{AIContext: AIContext{Engine: t.Engine, Person: person}})
	} else {
		ai.PopState()
		spawnPos := ai.StartPosition
		startLookDir := ai.StartLookDirection
		ai.PushState(&GotoBehaviour{
			AIContext:      AIContext{Engine: t.Engine, Person: person},
			TargetLocation: spawnPos,
			CallOnArrival: func() {
				person.LookDirection = startLookDir
			},
		})
