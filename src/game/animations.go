...
// MeleeSignalAnimation shows GlyphFourPointStar at the attacker's position for
// windowSecs – colour-coded by attack type – then fires onAttack. Cancels
// (calls onCancel instead) if the attacker moves or is incapacitated.
func (a *Animator) MeleeSignalAnimation(attacker *core.Actor, attackType core.MeleeAttackType, windowSecs float64, onAttack func(), onCancel func()) {
	pos := attacker.Pos()
	color := attackType.Color()
	drawFunc := func(grid console.CellInterface, _ int) {
		if !a.engine.GetGame().IsOnScreen(pos) {
			return
		}
		cellAt := a.cellOnScreen(grid, pos)
		style := cellAt.Style
		style.Foreground = color
		a.drawWorldToScreen(grid, pos, common.Cell{Rune: core.GlyphFourPointStar, Style: style})
	}
	frameDelayInTicks := uint64(utils.SecondsToTicks(windowSecs))
	animation := &ActiveAnimation{
		nextFrame:        func(_ int, ticksAlive uint64) bool { return ticksAlive >= frameDelayInTicks },
		DrawFrame:        drawFunc,
		frameCount:       1,
		FinishedCallback: onAttack,
		CancelCondition:  ActorMovedOrIncapacitated(attacker),
...
func (a *Animator) SleepingAnimation(person *core.Actor, finishedCallback func()) {
	...
	frameDelay := uint64(utils.SecondsToTicks(2))
	nextFrameFunc := func(frameIndex int, ticksAlive uint64) bool {
		return ticksAlive >= frameDelay
	}
	animation := &ActiveAnimation{
