...
func (w WakeUpAction) Description(services.Engine, *core.Actor, geometry.Point) (rune, common.Style) {
	return core.GlyphEmptyHand, common.DefaultStyle.WithBg(core.CurrentTheme.LegalActionBackground)
}

func (w WakeUpAction) Action(m services.Engine, person *core.Actor, position geometry.Point) {
	actorAt := m.GetGame().GetMap().DownedActorAt(position)
	if actorAt == nil {
		return
	}
	m.GetGame().WakeUp(actorAt)
}

func (w WakeUpAction) IsActionPossible(m services.Engine, person *core.Actor, actionAt geometry.Point) bool {
	actorAt := m.GetGame().GetMap().DownedActorAt(actionAt)
	return actorAt != nil && actorAt.IsSleeping() && actorAt.IsAlive() && person.Pos() != actionAt
}
...
type AssassinationAction struct{}

func (a AssassinationAction) Description(m services.Engine, person *core.Actor, _ geometry.Point) (rune, common.Style) {
	icon := core.GlyphEmptyHand
	if person.EquippedItem != nil && person.EquippedItem.HasMeleePiercingDamage() {
		icon = person.EquippedItem.Icon()
	}
	return icon, common.DefaultStyle.WithBg(core.CurrentTheme.IllegalActionBackground)
}

func (a AssassinationAction) Action(m services.Engine, person *core.Actor, position geometry.Point) {
	game := m.GetGame()
	victim := game.GetMap().ActorAt(position)
	if victim == nil {
		return
	}

	var weapon *core.Item
	var cod core.CoDDescription = core.CoDStrangled
	if person.EquippedItem != nil && person.EquippedItem.HasMeleePiercingDamage() {
		weapon = person.EquippedItem
		cod = core.CoDStabbed
	}
	...
	finished := func() {
		...
		game.IllegalActionAt(capturedVictim.Pos(), core.ObservationPersonAttacked)
		game.Kill(capturedVictim, core.CauseOfDeath{
			Description: capturedCod,
			Source:      core.EffectSource{Actor: person, Item: capturedWeapon},
		})
	}
	m.GetAnimator().AssassinationAnimation([]*core.Actor{victim}, icon, finished)
}

func (a AssassinationAction) IsActionPossible(m services.Engine, person *core.Actor, actionAt geometry.Point) bool {
	currentMap := m.GetGame().GetMap()
	actorAt := currentMap.ActorAt(actionAt)
	if actorAt == nil || actorAt == person {
		return false
