...
// WordWrapLines wraps each StyledText entry so that no line exceeds maxWidth
// characters. Entries that are too wide are split at word boundaries (using
// Format) and expanded into multiple StyledText entries that share the same
// style. Entries that already fit are passed through unchanged.
func WordWrapLines(lines []StyledText, maxWidth int) []StyledText {
    if maxWidth <= 0 {
        return lines
    }
    result := make([]StyledText, 0, len(lines))
    for _, line := range lines {
        if line.Size().X <= maxWidth {
            result = append(result, line)
            continue
        }
        wrapped := line.Format(maxWidth - 1)
        parts := strings.Split(wrapped.text, "\n")
        for index, part := range parts {
            lineText := part
            if index > 0 {
                lineText = " " + lineText
            }
            result = append(result, StyledText{text: lineText, style: wrapped.style, markups: wrapped.markups})
        }
    }
    return result
}
