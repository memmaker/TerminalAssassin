...
func (pr *PathRange) JPSPath(path []Point, from, to Point, passable func(Point) bool, diags bool) []Point {
...
func (pr *PathRange) expandOrigin(from, to Point) {
	for y := -1; y <= 1; y++ {
		for x := -1; x <= 1; x++ {
			if x == 0 && y == 0 {
				continue
			...
			if !pr.diags {
				if dir.X != 0 && dir.Y != 0 {
					if pr.pass(from.Add(Point{dir.X, 0})) || pr.pass(from.Add(Point{0, dir.Y})) {
...
func (pr *PathRange) pass(p Point) bool {
	return p.In(pr.Rg) && pr.passable(p)
}

func (pr *PathRange) obstacle(p Point) bool {
	return p.In(pr.Rg) && !pr.passable(p)
}

func right(p Point, dir Point) Point {
	return Point{p.X - dir.Y, p.Y + dir.X}
}

func left(p Point, dir Point) Point {
	return Point{p.X + dir.Y, p.Y - dir.X}
}
...
func (pr *PathRange) straightMax(p, dir Point) (int, forcedSucc) {
	fs := fsBoth
	max := 0
	switch {
	case dir.X > 0:
...
func (pr *PathRange) jumpStraight(p, dir, to Point) (Point, int) {
...
func (pr *PathRange) jumpStraightNone(p, dir, to Point, max int) (Point, int) {
	for i := 1; i < max+1; i++ {
		if !pr.passable(p) {
			return p, 0
		}
...
func (pr *PathRange) jumpStraightLeft(p, dir, to Point, max int) (Point, int) {
	for i := 1; i < max+1; i++ {
		if !pr.passable(p) {
			return p, 0
		}
...
func (pr *PathRange) jumpStraightRight(p, dir, to Point, max int) (Point, int) {
	for i := 1; i < max+1; i++ {
		if !pr.passable(p) {
			return p, 0
		}
...
func (pr *PathRange) jumpStraightNoDiags(p, dir, to Point) (Point, int) {
	max, fs := pr.straightMax(p, dir)
	switch fs {
	case fsNone:
		return pr.jumpStraightNone(p, dir, to, max)
...
func (pr *PathRange) jumpStraightLeftNoDiags(p, dir, to Point, max int) (Point, int) {
	for i := 1; i < max+1; i++ {
		if !pr.passable(p) {
			return p, 0
		}
...
func (pr *PathRange) jumpStraightRightNoDiags(p, dir, to Point, max int) (Point, int) {
	for i := 1; i < max+1; i++ {
		if !pr.passable(p) {
			return p, 0
		}
...
func (pr *PathRange) jumpDiagonal(p, dir, to Point, cost int) (Point, int) {
	i := 1
	from := p.Sub(dir)
	for {
		if !pr.pass(p) {
		...
		if q := p.Shift(-dir.X, 0); pr.obstacle(q) {
			if pr.pass(p.Add(Point{-dir.X, dir.Y})) {
				return p, i
			}
		}
		...
		q, j := pr.jumpStraight(p.Shift(dir.X, 0), Point{dir.X, 0}, to)
		if j > 0 {
			pr.addSuccessor(q, from, to, cost+i+j)
		}
		q, j = pr.jumpStraight(p.Shift(0, dir.Y), Point{0, dir.Y}, to)
		if j > 0 {
			pr.addSuccessor(q, from, to, cost+i+j)
		}
		p = p.Add(dir)
...
func (pr *PathRange) jumpDiagonalNoDiags(p, dir, to Point, cost int) (Point, int) {
	...
	for {
		...
		if !pxpass {
			if pr.pass(p.Add(Point{-dir.X, dir.Y})) && pr.pass(p.Add(Point{0, dir.Y})) {
				return p, i
			}
		}
		if !pypass {
			if pr.pass(p.Add(Point{dir.X, -dir.Y})) && pr.pass(p.Add(Point{dir.X, 0})) {
				return p, i
			}
		}
...
// open list.
func (pr *PathRange) jump(p, dir, to Point, cost int) (Point, int) {
	switch {
...
func (pr *PathRange) jumpNoDiags(p, dir, to Point, cost int) (Point, int) {
	switch {
	case dir.X == 0 || dir.Y == 0:
...
// jump).
func dirnorm(p, q Point) Point {
	dir := q.Sub(p)
	dx := Abs(dir.X)
	dy := Abs(dir.Y)
	dir = Point{sign(dir.X), sign(dir.Y)}
	switch {
	case dx == dy:
	case dx > dy:
		dir.Y = 0
...
func (pr *PathRange) neighbors(neighbors []Point, n *node, to Point) []Point {
	...
	default:
		...
		if q := n.P.Shift(-dir.X, 0); !pr.pass(q) {
			if pr.diags || pr.pass(n.P.Add(Point{0, dir.Y})) {
				cost := diagCost(pr.diags)
				pr.addSuccessor(q.Shift(0, dir.Y), n.P, to, n.Cost+cost)
			}
		}
		if q := n.P.Shift(0, -dir.Y); !pr.pass(q) {
			if pr.diags || pr.pass(n.P.Add(Point{dir.X, 0})) {
				cost := diagCost(pr.diags)
...
func (pr *PathRange) addSuccessor(p, parent, to Point, cost int) {
	if !pr.pass(p) {
		return
	}
...
func (pr *PathRange) jumpPath(path []Point, p, q Point) []Point {
	dir := q.Sub(p)
	dx := Abs(dir.X)
	dy := Abs(dir.Y)
	dir = Point{sign(dir.X), sign(dir.Y)}
	...
	case dx > dy:
		for i := 0; i < dx-dy; i++ {
			path = append(path, p)
			p = p.Add(Point{dir.X, 0})
		}
	case dx < dy:
		for i := 0; i < dy-dx; i++ {
			path = append(path, p)
			p = p.Add(Point{0, dir.Y})
		}
	}
	for ; p != q; p = p.Add(dir) {
		path = append(path, p)
		if !pr.diags {
			if dir.X != 0 && dir.Y != 0 {
				if px := p.Add(Point{dir.X, 0}); pr.pass(px) {
					path = append(path, px)
				} else if py := p.Add(Point{0, dir.Y}); pr.pass(py) {
					path = append(path, py)
				}
			}
		}
...
func (pr *PathRange) path(path []Point, from Point, n *node) []Point {
	for {
		if n.P == from {
