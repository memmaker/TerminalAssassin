...
func main() {
	...
	gridConfig := console.OptimalGridConfig(50, 10)
	opts := LoadOptions(gridConfig.TileSize)
	...
	con := console.NewConsole(gridConfig)
	...
	ebiten.SetWindowSize(opts.WindowedWidth, opts.WindowedHeight)
	if opts.Fullscreen {
		ebiten.SetFullscreen(true)
	}
	ebiten.SetScreenClearedEveryFrame(false)
	consoleGame.Init()
	// Apply persisted options.
	consoleGame.Audio.SetMasterVolume(opts.MasterVolume)
	consoleGame.Audio.SetMusicVolume(opts.MusicVolume)
	consoleGame.Audio.SetSoundVolume(opts.SoundVolume)
	if opts.TextFont != "" {
		consoleGame.Console.SetHalfWidthFont(opts.TextFont)
	}
...
func (g *ConsoleEngine) LayoutF(outsideWidth, outsideHeight float64) (screenWidth, screenHeight float64) {
	...
	if !g.GetGame().GetConfig().Fullscreen {
		...
		if newW > 0 && newH > 0 && (newW != g.options.WindowedWidth || newH != g.options.WindowedHeight) {
			g.options.WindowedWidth = newW
			g.options.WindowedHeight = newH
		}
	}
...
func (g *ConsoleEngine) SetFullscreen(enabled bool) {
	...
	g.options.Fullscreen = enabled
	ebiten.SetFullscreen(enabled)
	g.Console.ClearSurface()
	g.Console.ClearConsole()
	g.SaveOptions()
}

func (g *ConsoleEngine) SetControllerMode(mode string) {
	g.GetGame().GetConfig().ControllerMode = mode
	g.Input.SetControllerMode(mode)
	g.Input.SaveKeyDefs()
	g.SaveOptions()
}

func (g *ConsoleEngine) SaveOptions() {
	g.options.MasterVolume = g.Audio.GetMasterVolume()
	g.options.MusicVolume = g.Audio.GetMusicVolume()
	g.options.SoundVolume = g.Audio.GetSoundVolume()
	g.options.ShowHints = g.GetGame().GetConfig().ShowHints
	g.options.ControllerMode = g.GetGame().GetConfig().ControllerMode
